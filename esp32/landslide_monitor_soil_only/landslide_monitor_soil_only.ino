...
// ===== WiFi Configuration =====
const char* ssid = "Ritik";                     // Your WiFi name
const char* password = "rrrrrrrr";              // Your WiFi password
...
bool firstRun = true; // Flag to send data immediately on boot
...
void loop() {
  ...
  // Send to cloud every 30 seconds (and immediately on first boot)
  unsigned long currentTime = millis();
  if (firstRun || (currentTime - lastSendTime >= sendInterval)) {
    if (WiFi.status() == WL_CONNECTED) {
      sendDataToCloud(soilMoisture);
      lastSendTime = currentTime;
      firstRun = false; // Clear the flag after first send
    } else {
...
void sendDataToCloud(float soilMoisture) {
  ...
  http.setTimeout(20000); // 20 second timeout to allow server wake up
  ...
  if (httpResponseCode > 0) {
    ...
    if (httpResponseCode == 201) {
      ...
      Serial.println("⏳ Server is waking up (Render free tier)...");
      Serial.println("🔄 Retrying in 10 seconds...");
      http.end();
      
      delay(10000); // Wait 10 seconds for server to wake up
      
      // Retry once
      http.begin(serverUrl);
      http.addHeader("Content-Type", "application/json");
      http.setTimeout(20000);
      int retryCode = http.POST(jsonString);
      
      if (retryCode == 201) {
        Serial.println("✅ Retry successful! Data saved!");
        String response = http.getString();
        Serial.println(response);
      } else {
        Serial.print("❌ Retry failed. Code: ");
