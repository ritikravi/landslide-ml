...
// Sentinel Hub - For NDVI and terrain analysis (requires API key)
const SENTINEL_BASE = 'https://services.sentinel-hub.com/api/v1';

// GPM IMERG - Rainfall data
const GPM_BASE = 'https://gpm1.gesdisc.eosdis.nasa.gov/data/GPM_L3';
...
class SatelliteMlService {
  ...
  /**
   * Get rainfall data from multiple sources
   */
  async getRainfallData(lat, lon) {
    try {
      // Get last 7 days rainfall
      ...
      return {
        ...
        last30days: values.reduce((a, b) => a + b, 0),
  ...
  async getTerrainData(lat, lon) {
    try {
      ...
      // Calculate approximate slope from surrounding elevations
      const slope = await this.calculateSlope(lat, lon, elevation);

      return {
        elevation,
        slope,
        aspect: this.calculateAspect(lat, lon) // Approximate
      };
    } catch (error) {
  ...
  /**
   * Calculate slope from surrounding elevations
   */
  async calculateSlope(lat, lon, centerElevation) {
    try {
      ...
      return Math.min(slope, 45); // Cap at 45 degrees
    } catch {
      return 5; // Default flat terrain
    }
  }
  ...
  calculateAspect(lat, lon) {
    // Simple approximation based on hemisphere
    if (lat > 0) {
      return 180; // South-facing in northern hemisphere
    } else {
      return 0; // North-facing in southern hemisphere
    }
  }
  ...
  /**
   * Generate landslide risk prediction from satellite data
   */
  ...
  assessRainfallRisk(rainfall) {
    ...
    if (rainfall.last24h > 50) {
      ...
      score += 15;
      factors.push('Moderate rainfall in last 24 hours');
    }

    // 7-day accumulation
    if (rainfall.last7days > 200) {
      score += 40;
      factors.push('Extreme 7-day rainfall accumulation');
    } else if (rainfall.last7days > 100) {
      score += 25;
      factors.push('High 7-day rainfall accumulation');
    }

    // Trend
    if (rainfall.trend === 'increasing') {
      score += 10;
      factors.push('Rainfall intensity increasing');
    }
  ...
  assessTerrainRisk(terrain) {
    ...
    // Elevation (high elevation areas in mountains)
    if (terrain.elevation > 2000) {
      score += 20;
      factors.push(`High elevation (${terrain.elevation}m) - mountainous region`);
  ...
  assessClimateRisk(climate) {
    ...
    if (climate.humidity > 85) {
      score += 15;
      factors.push('Very high humidity - saturated conditions');
    }
  ...
  /**
   * Assess sensor risk (if available)
   */
  assessSensorRisk(sensorData) {
    ...
    if (sensorData.tilt > 10) {
      score += 30;
      factors.push('Significant ground tilt detected');
    }

    if (sensorData.vibration > 30) {
      score += 20;
      factors.push('High vibration activity');
    }
  ...
  /**
   * Calculate overall risk from all factors
   */
  calculateOverallRisk(factors) {
    ...
    // Adjust weights if sensor data not available
    if (!factors.sensor) {
      weights = {
        rainfall: 0.45,
        terrain: 0.35,
    ...
    // Increase confidence if sensor data is available
    if (factors.sensor) {
      confidence += 10;
    }
...
module.exports = new SatelliteMlService();
